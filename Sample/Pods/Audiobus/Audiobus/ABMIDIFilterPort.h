 ...
 *
 *  See ABMIDIFilterPort's [MIDIFlushEventBlock](ABMIDIFilterPort::MIDIFlushEventBlock) documentation
 *  for details.
 *
 *  Note: this will be called on the realtime MIDI receive thread,
 *  so be careful not to do anything that could cause priority inversion,
 ...
 *  to reduce latency or jitter effects. When this occurs, filters will see incoming
 *  MIDI events with timestamps in the future, and should take steps to schedule
 *  MIDI events accordingly.
 *
 *  To facilite elegant transport stop, MIDI sources will initiate a flush (see ABMIDIPortFlushOutput),
 *  which you may handle by assigning a block to this property. The block will be called
 *  when a flush occurs, and you should respond by unscheduling any pending events. Any
 *  MIDI messages already dispatched via ABMIDIPortSendPacketList which have a timestamp in
 *  the future will be automatically removed.
 *
 *  Note: this will be called on the realtime MIDI receive thread,
 *  so be careful not to do anything that could cause priority inversion,
 *  like calling Objective-C, allocating memory, or holding locks.
 */
@property (nonatomic, copy) ABMIDIFilterPortMIDIFlushEventBlock _Nullable MIDIFlushEventBlock;
