Audiobus SDK -- Version 3.0.12 -- Aug 10 2020
============================================
...
----
 ...
 - On iOS 14, the SDK will delay starting network communications until it detects Audiobus as a host, unless your app has an audio receiver port (see ABAudiobusController's startNetworkCommunications method for instructions, in this case).
 - Adds prepareForNetworkCommunicationsBlock property on ABAudiobusController, for optionally preparing user to see local networking permission prompt.
 - Fixed noisy-but-harmless ABAudioFilterPort errors when bypass enabled with sample rate conversion.

3.0.11
----

 - Fixed connection panel issue with scene-based apps on iOS 13
 - This version no longer compatible with Xcode 10 - use 3.0.10 for Xcode 10 builds.
 
3.0.10
----

 - Fixed a potential crash when accessing peer icons on iOS 13
 - Fixed an internal issue with advertising multi-instance ports (workaround already present in Audiobus v3.4.13)

3.0.9
----

 - Fixed an issue where the connection panel could block views underneath on iOS 13 on the iPad Pro 11".
 - Fixed connection panel safe area layout (e.g. on iPhone X and up) when scrolling.
 - Fixed misplaced connection panel scroll indicators on iOS 13

3.0.8
----

 - Fixed an issue where an empty connection panel could spuriously appear when swiped in from the side of the screen when connected only to IAA (and not Audiobus).
